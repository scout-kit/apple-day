 ...
 *
 * The by-location table says Braemar did well without saying when; the by-hour table says
...
const LOCATIONS = [location('braemar', 'Braemar'), location('kelmont', 'Kelmont')]
...
describe('reading the grid across', () => {
  const assignments = [
    ...
    shift('a2', 'fri-1800', 'braemar'),
    shift('b1', 'fri-1700', 'kelmont'),
  ]
  const jars = [
    jar({ id: 'j1', locationId: 'braemar', assignmentId: 'a1', assignmentIds: ['a1'], amount: 300 }),
    jar({ id: 'j2', locationId: 'braemar', assignmentId: 'a2', assignmentIds: ['a2'], amount: 50 }),
    jar({ id: 'j3', locationId: 'kelmont', assignmentId: 'b1', assignmentIds: ['b1'], amount: 80 }),
  ]

  it('puts each hour’s money in its own cell', () => {
    const grid = locationHourGrid(LOCATIONS, assignments, jars, SLOTS)
    expect(cell(grid, 'braemar', 'fri-1700').revenue).toBe(300)
    expect(cell(grid, 'braemar', 'fri-1800').revenue).toBe(50)
    expect(cell(grid, 'kelmont', 'fri-1700').revenue).toBe(80)
  })

  it('leaves an unstaffed hour empty rather than blank-looking-like-zero-earned', () => {
    const grid = locationHourGrid(LOCATIONS, assignments, jars, SLOTS)
    const quiet = cell(grid, 'kelmont', 'fri-1800')
  ...
  it('names each location’s best hour', () => {
    const grid = locationHourGrid(LOCATIONS, assignments, jars, SLOTS)
    expect(grid.rows.find((r) => r.locationId === 'braemar')!.bestSlotId).toBe('fri-1700')
  })
  ...
  it('keeps the year’s own running order, whatever the takings', () => {
    ...
    const quiet = location('kelmont', 'Kelmont')
    const busy = location('braemar', 'Braemar')
    const grid = locationHourGrid([quiet, busy], assignments, jars, SLOTS)
    expect(grid.rows.map((r) => r.locationId)).toEqual(['kelmont', 'braemar'])
    // Even though Braemar took far more.
  ...
  it('puts a location that only appears in the data after the year’s own', () => {
    ...
    expect(grid.rows.map((r) => r.locationId)).toEqual(['braemar', 'kelmont', 'staff-lounge'])
  })
})
...
describe('reading the grid down', () => {
  it('totals each hour across every location', () => {
    ...
    const grid = locationHourGrid(
      ...
      assignments,
      [
        jar({ id: 'j1', locationId: 'braemar', assignmentId: 'a1', assignmentIds: ['a1'], amount: 100 }),
        jar({ id: 'j2', locationId: 'kelmont', assignmentId: 'b1', assignmentIds: ['b1'], amount: 60 }),
      ],
      SLOTS,
    )
  ...
  it('reconciles: every cell adds up to every row total', () => {
    const assignments = [shift('a1', 'fri-1700', 'braemar'), shift('a2', 'fri-1800', 'braemar')]
    const grid = locationHourGrid(
      LOCATIONS,
      assignments,
      [jar({ id: 'j1', locationId: 'braemar', assignmentId: 'a1', assignmentIds: ['a1', 'a2'], amount: 100 })],
      SLOTS,
    )
...
describe('a jar carried across two hours', () => {
  it('splits into both cells of the same location', () => {
    ...
    const grid = locationHourGrid(
      LOCATIONS,
      assignments,
      [jar({ id: 'j1', locationId: 'braemar', assignmentId: 'a1', assignmentIds: ['a1', 'a2'], amount: 200 })],
      SLOTS,
    )
    expect(cell(grid, 'braemar', 'fri-1700').revenue).toBe(100)
    expect(cell(grid, 'braemar', 'fri-1800').revenue).toBe(100)
  })
})
...
describe('nothing hides from the grid', () => {
  ...
  it('includes a location the year uses but never staffed', () => {
    ...
    expect(grid.rows.map((r) => r.locationId).sort()).toEqual(['braemar', 'kelmont'])
  })
})
